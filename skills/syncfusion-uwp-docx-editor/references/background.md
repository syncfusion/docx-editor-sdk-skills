...
Background customization in the Syncfusion UWP DOCX Editor allows controlling the visual appearance of the editor control and document pages.
...
**Placeholders**
- `new SolidColorBrush(Color.FromArgb(255,102, 153, 204))` → Replace with `{backgroundBrush}`
- `Color.FromArgb(255,102, 153, 204)` → Replace with `{backgroundColor}`
...
## XAML
```xaml
<RichTextBoxAdv:SfRichTextBoxAdv x:Name="richTextBoxAdv" Background="#6699cc" OverridesDocumentBackground="True" />
```
...
**Placeholders**
- `Color.FromArgb(255,102, 153, 204)` → Replace with `{documentBackgroundColor}`
---