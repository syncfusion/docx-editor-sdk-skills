# Character Formatting in WPF DOCX Editor

Character formatting support in the Syncfusion WPF DOCX Editor allows applying and retrieving character‑level text formatting using the `Selection.CharacterFormat` API.
...
```csharp
 SfRichTextBoxAdv.ClearFormattingCommand.Execute(null, richTextBoxAdv);
```
