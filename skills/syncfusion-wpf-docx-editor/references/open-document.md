# Opening Documents in WPF DOCX Editor

Document opening support in the Syncfusion WPF DOCX Editor allows loading documents into the editor using built‑in APIs and commands.

---

## Supported formats
Document formats supported by the WPF Document editor:
...
```xaml
<Button Content="Open" Click="OpenDocument_Click" />
...
private void OpenDocument_Click(object sender, RoutedEventArgs e)
{
    ...
    dlg.Filter = "Word Document (*.docx)|*.docx|Word 97 - 2003 Document (*.doc)|*.doc|Web Page (*.htm;*.html)|*.htm;*.html|Rich Text File (*.rtf)|*.rtf|Text File (*.txt)|*.txt|Xaml File (*.xaml)|*.xaml";
...
**Placeholders**
- `"Word Document (*.docx)|*.docx|Word 97 - 2003 Document (*.doc)|*.doc|Web Page (*.htm;*.html)|*.htm;*.html|Rich Text File (*.rtf)|*.rtf|Text File (*.txt)|*.txt|Xaml File (*.xaml)|*.xaml"`
  → Replace with `{openFileDialogFilter}`

- `openFileDialog.FileName` → Replace with `{filePath}`
...
```csharp
// Load from a Syncfusion DocIO WordDocument instance into WPF SfRichTextBoxAdv.
using (WordDocument wordDocument = new WordDocument("sample.docx"))
{
    richTextBoxAdv.Load(wordDocument);
}
```
