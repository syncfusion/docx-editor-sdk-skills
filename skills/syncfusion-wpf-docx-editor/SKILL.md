...
5. **Only after the user selects a delivery mode**, proceed to generate WPF code using the APIs and snippets from `references/*.md`, substituting concrete placeholders from the user's project.
...
7. Provide complete WPF snippets and concise integration steps after delivering the code.
...
| **localization.md** | Apply localization using `.resx` resources and UI culture settings. |
...
2. **Snippets must be tested** — All code must build and run against current Syncfusion WPF NuGet packages (e.g., Syncfusion.SfRichTextBoxAdv.WPF) and supported .NET target frameworks.
...
7. **Framework adaptation** — Default to C# + XAML. When user specifies framework.
