...
documentEditor.serviceUrl = '{backendServiceUrl}';
```
...
- `Program.cs` (or `Startup.cs`) → Register Syncfusion services with `services.AddSyncfusionEJ2()` and configure the DocumentEditor route
- `Controllers/DocumentEditorController.cs` → Ensure controller exists and inherits from Syncfusion base controller
- `Frontend (cshtml)` → Replace `{backendServiceUrl}` with your actual backend API URL (e.g., `https://localhost:5001/api/documenteditor`)
